...
<body>
	<div class="avatar-holder">
		...
		<div class="container" style="padding: 0px 0px 0px 110px;">
			<div style="width: 1000px; padding: 20px 0px 0px 50px; border: 1px solid #C5C1AA">
				...
				<!-- 下面是图片和文字的内容 -->
				<c:choose>
					<c:when test = "${empty searchFruitByName}">
					<div style="height:300px;text-align:center;">
						<p style="padding:50px 0px 0px 0px">未搜索到相关内容</p>
					</div>
					</c:when>
					<c:otherwise>
						<c:forEach items="${searchFruitByName}" var="searchFruitVM">
					...
					<div style = "height:150px;padding:0px 0px 0px 220px;">
						...
						<tr>
						<c:choose>
							<c:when test="${searchFruitVM.pregnantEat=='Y'}">
						...
						<c:choose>
							<c:when test="${searchFruitVM.momEat=='Y'}">
						...
						<c:choose>
							<c:when test="${searchFruitVM.babyEat=='Y'}">
					...
					<!-- 下面是孕妇产妇能吃不能吃的原因 -->
						<div>
							...
							<p>${searchFruitVM.reason.pregnanteat}</p>
						 	</div>
						</div>
						<hr>
						<div>
						 	...
						 	<p> ${searchFruitVM.reason.momeat}</p>
						</div>
						<hr>
						<div>
						 		<c:choose>
						 	...
						 	<p> ${searchFruitVM.reason.babyeat}</p>
						</div>
						<hr>
					<!-- 下面是营养成分 -->
	...
	<div class="row clearfix">
			...
			<thead>
				...
				<c:forEach items="${searchFruitVM.nutritions}" var="nutritions">
					<tr>
