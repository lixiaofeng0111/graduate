...
<script type="text/javascript">
	$(document).ready(function(){
		/** 新增   **/
	    $("#addBtn").fancybox({
	    	'href'  : 'admin/addpregnantAnalysis.jsp',
	    	'width' : 733,
	        'height' : 530,
	        'type' : 'iframe',
	        ...
	        'onClosed' : function() { 
	        	window.location.href = '/pregnantAnalysis';
	        }
	    });
	...
	function batchDel(){
    //判断至少选择了一项
    var checkedNum = $("input[name='analysisCheck']:checked").length;
    if (checkedNum == 0) {
    ...
    if (confirm("确定删除选中的用户？")) {
        ...
        $("input[name='analysisCheck']:checked").each(function(){
            analysisList.push($(this).val());
        });
    $.ajax({
            type : "post",
            url : "/batchdeletepregnantAnalysisById",
            data : {analysisList : analysisList.toString()},
            success : function(){
            	alert("批量删除成功！");
                location.reload();
            },
			...
			<div class="ui_content">
				<div class="ui_tb">
					<table class="table" cellspacing="0" cellpadding="0" width="100%" align="center" border="0">
						...
						<tr></tr>
						<c:forEach items="${pregnantAnalysis}" var="Analysis">
							<tr>
				...
				<div style="text-align: center;">
					<ul class="pagination">
						<c:choose>
							...
							<c:otherwise>
								<li><a href="/pregnantAnalysis?page=${currentPage - 1} ">上一页</a></li>
							</c:otherwise>
						</c:choose>
						<c:forEach begin="1" end="${totalPage}" var="index">
							<c:choose>
								...
								<c:otherwise>
									<li><a href="/pregnantAnalysis?page=${index}">${index}</a></li>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						<c:choose>
							...
							<c:otherwise>
								<li><a href="/pregnantAnalysis?page=${currentPage + 1} ">下一页</a></li>
							</c:otherwise>
						</c:choose>
					</ul>
				</div>
