...
<script type="text/javascript">
	$(document).ready(function(){
		/** 新增   **/
	    $("#addBtn").fancybox({
	    	'href'  : 'admin/addpregnantAnalysisMeitushow.jsp',
	    	'width' : 733,
	        'height' : 530,
	        'type' : 'iframe',
	        ...
	        'onClosed' : function() { 
	        	window.location.href = '/pregnantAnalysisMeitushow';
	        }
	    });
	...
	function batchDel(){
    //判断至少选择了一项
    var checkedNum = $("input[name='MeituCheck']:checked").length;
    if (checkedNum == 0) {
    	art.dialog({icon:'error', title:'友情提示', drag:false, resize:false, content:'至少选择一条', ok:true,});
        return;
    }
    if (confirm("确定删除选中的用户？")) {
        var meituList = new Array();
        $("input[name='MeituCheck']:checked").each(function(){
            meituList.push($(this).val());
        });
    $.ajax({
            type : "post",
            url : "/batchdeleteMeituById",
            data : {meituList : meituList.toString()},
            success : function(){
            	alert("批量删除成功！");
                location.reload();
            },
				...
				<div style="text-align: center;">
					<ul class="pagination">
						<c:choose>
							...
							<c:otherwise>
								<li><a href="/pregnantAnalysisMeitushow?page=${currentPage - 1} ">上一页</a></li>
							</c:otherwise>
						</c:choose>
						<c:forEach begin="1" end="${totalPage}" var="index">
							<c:choose>
								...
								<c:otherwise>
									<li><a href="/pregnantAnalysisMeitushow?page=${index}">${index}</a></li>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						<c:choose>
							...
							<c:otherwise>
								<li><a href="/pregnantAnalysisMeitushow?page=${currentPage + 1} ">下一页</a></li>
							</c:otherwise>
						</c:choose>
					</ul>
				</div>
